	...
	$('.add-new-blank').click(function(){
		...
		var $htm = '\
			<tr onclick="id_delete(' + $id + ');" class="item-barang" data-item="' + $id + '">\
				<td>\
					<input type="hidden" value="0" name="id_barang[]">\
				</td>\
		...
		$.getJSON(_base_url + '/fakturpembelian/additem', {id : id}, function(json){
			var $htm = '\
				<tr onclick="id_delete(' + json.id_barang + ');" class="item-barang" data-item="' + json.id_barang + '">\
					...
					<td>\
						<input type="text" value="' + json.nm_barang + '" name="deskripsi[]" readonly="readonly" class="form-control" required>\
						<input type="hidden" value="' + json.nm_barang + '" name="nm_barang[]">\
					</td>\
					<td>\
						<div class="input-group input-group-sm">\
							<input type="number" data-form="qty" value="1" name="qty[]" class="form-control text-right" required>\
						  	...
						  	<input type="hidden" name="id_coa[]" value="' + json.id_coa + '" />\
						</div>\
					</td>\
			...
			for(var i =0; i < json.items.length; i++){
				$htm += '\
					<tr data-po="true" class="item-barang">\
						...
						<td>\
							<input type="text" value="' + json.items[i].nm_barang + '"  name="deskripsi[]" readonly="readonly" class="form-control" required>\
							<input type="hidden" value="' + json.items[i].nm_barang + '" name="nm_barang[]">\
						</td>\
						<td>\
							<div class="input-group input-group-sm">\
								<input type="number" data-form="qty" value="' + json.items[i].qty + '" name="qty[]" class="form-control text-right" required>\
							  	...
							  	<input type="hidden" name="id_coa[]" value="' + json.items[i].id_coa + '" />\
							</div>\
						</td>\
