...
use App\Models\Views\view_count_po;
    ...
    public function handle() {
        ...
        try{
            ...
            foreach($items as $item){
                ...
                // update Harga
                $brg = data_barang::find($item->id_item);

                /* Konversi Harga */
                $konersi = \Format::convertSatuan($item->id_item, $item->id_satuan, $brg->id_satuan);
                $harga = $item->harga / $konersi;
            ...
            if(date('Y') > 2015):
                $data = view_count_po::where('tahun', date('Y'))->first();
                $urut = empty($data->jumlah) ? 1 : $data->jumlah;
                $po->no_po = 'PO/' . date('Y') . '/' . \Format::code($urut);
                $po->save();
            else:
                $data = view_count_po::where('tahun', date('Y'))->first();
                $urut = empty($data->jumlah) ? 1 : $data->jumlah;
                $po->no_po = 'PO-' . \Format::code($urut);
                $po->save();
            endif;
