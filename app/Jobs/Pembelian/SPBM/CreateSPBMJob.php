...
use App\Models\Views\view_count_gr;
       ...
       try{
            ...
            foreach($this->req['id_po_item'] as $i => $id_po_item){
                if(!empty($this->req['qty_lg'])){
                    ...
                    // Log Stok
                    $bonus = $this->req['bonus'][$i] == 1 ? ' Bonus item' : '';
                    data_log_barang::create([
                        'id_barang' => $this->req['id_barang'][$i],
                        'qty' => $qty,
                        'keterangan' => 'Good Receive' . $bonus,
                        'id_gudang' => 0,
                        'kondisi' => 1,
                        'tipe' => 2,
            ...
            if(date('Y') > 2015):
                $data = view_count_gr::where('tahun', date('Y'))->first();
                $urut = empty($data->jumlah) ? 1 : $data->jumlah;
                $gr->no_spbm = 'GR/' . date('Y') . '/' . \Format::code($urut);
                $gr->save();
            else:
                $data = view_count_gr::where('tahun', date('Y'))->first();
                $urut = empty($data->jumlah) ? 1 : $data->jumlah;
                $gr->no_spbm = 'GR-' . \Format::code($urut);
                $gr->save();
            endif;
