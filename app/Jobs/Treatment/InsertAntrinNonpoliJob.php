...
use App\Models\data_antrian_non_poli;

use App\Jobs\Job;
use Illuminate\Contracts\Bus\SelfHandling;

class InsertAntrinNonpoliJob extends Job implements SelfHandling
{
    public $req;
    /**
    ...
    public function handle()
    {
         try{
             \DB::begintransaction();
               $pa = data_antrian_non_poli::firstOrCreate([
                    'no_antrian'                 =>$this->pa->NO_ANTRIAN_NON_POLI,
                    'tgl_nonpoli'                =>$this->pa->TGL_NON_POLI,
                    'id_karyawan'                =>$this->pa->ID_PGW,
                    'peg_idpgw'                  =>$this->pa->peg_idPgw,
                    ...
                    'alasan_periksa'             =>$this->pa->ALASAN_PERIKSA_NON_POLI,
                    'proses'                     =>$this->pa->PROSES_NON_POLI,
                    'on_hold'                    =>$this->pa->ON_HOLD_NON_POLI,
                    'finish'                     =>$this->pa->FINISH_NON_POLI,
                    'status_report'              =>$this->pa->STATUS_REPORT_HC,
                    'tidak_hadir'                =>$this->pa->TIDAK_HADIR_NON_POLI,
                    'pembatalan'                 =>$this->pa->PEMBATALAN_NON_POLI,
                    'diagnosa'                   =>$this->pa->DIAGNOSA_HC,
                    'masalah'                    =>$this->pa->MASALAH_HC,
                    'waktu_hc'                   =>$this->pa->WAKTU_HC,
                    'keterangan'                 =>$this->pa->keterangan_NP,
                    'jam_np'                     =>$this->pa->jam_np,
                    'id_slot'                    =>$this->pa->id_slot_np,
                    'jam_konfirmasi'             =>$this->pa->jam_konfirmasi,
                    'jam_masuk'                  =>$this->pa->jam_masuk,
                    'flag_mr'                    =>$this->pa->flag_mr,
                ]);
