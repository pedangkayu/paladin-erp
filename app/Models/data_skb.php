...
class data_skb extends Model {
	...
	public function scopeListskb($query, $req = []){
		...
		if(count($req) > 0){
			if(!empty($req['no_spb']))
				$skb->where('data_spb.no_spb','LIKE', '%'. $req['no_spb'] . '%');
			if(!empty($req['no_skb']))
				$skb->where('data_skb.no_skb', 'LIKE', '%'. $req['no_skb'] . '%');
			if(!empty($req['dep']))
	...
	public function scopeByid($query, $id){
		return $query->join('data_spb', 'data_spb.id_spb', '=', 'data_skb.id_spb')
			...
			->join('data_karyawan', 'data_karyawan.id_karyawan', '=', 'data_skb.id_petugas')
			->join('data_skb_item', 'data_skb_item.id_skb', '=', 'data_skb.id_skb')
			->leftJoin('ref_gudang', 'ref_gudang.id_gudang', '=', 'data_skb_item.id_gudang')
