...
class data_spbm extends Model {
	...
	public function scopeFrombatch($query, $req = []){

		$gr = $query->join('data_po', 'data_po.id_po', '=', 'data_spbm.id_po')
			->join('data_vendor', 'data_vendor.id_vendor', '=', 'data_po.id_vendor')
			->join('data_karyawan', 'data_karyawan.id_karyawan', '=', 'data_spbm.id_karyawan')
			->leftJoin('data_batch','data_batch.id_spbm', '=', 'data_spbm.id_spbm');
		...
		$gr->select(
				\DB::raw('COUNT(data_batch.id_spbm) AS total_batch'),
				'data_spbm.*', 
				'data_po.no_po', 
				'data_vendor.nm_vendor',
				'data_karyawan.nm_depan',
			...
			->groupby('data_spbm.id_spbm')
			->orderby('data_spbm.id_spbm', 'desc');
		if(!empty($req['belum']) && $req['belum'] == 'true')
			$gr->having(\DB::raw('COUNT(data_batch.id_spbm)'), '=', 0);
