...
class data_barang extends Model{
	...
	protected $fillable = [
		...
		'out',
		'keterangan',
		'tipe', /* 1:Obat|2:Barang | 3:inventaris */
		'id_klasifikasi',
		'harga_beli',
		'ppn',
		'harga_jual',
		'reuse',
		'lokasi', /* -- tambahan 5 field untuk type inventaris */
		'merk',
		'dimensi',
		'kondisi',
		'tahun_beli',
	...
	/*Pengambil data barang untuk keperluan modul Permohonan barang*/
	...
	/*Pengambil data barang untuk keperluan modul Permohonan obat*/
	...
	public function scopeLaporanstok($query, $req = []){
		...
		$item = $query->leftJoin('ref_klasifikasi', 'ref_klasifikasi.id_klasifikasi', '=', 'data_barang.id_klasifikasi')
			->join('ref_kategori', 'ref_kategori.id_kategori', '=', 'data_barang.id_kategori')
			->join('ref_satuan', 'ref_satuan.id_satuan', '=', 'data_barang.id_satuan');
    ...
    /* BATCH */
	public function scopeMasterbatch($query, $req = []){
		$item = $query->join('data_batch', 'data_batch.id_barang', '=', 'data_barang.id_barang')
		...
		$item->select('data_barang.id_barang', 'data_barang.kode', 'data_barang.nm_barang')
			->groupby('data_barang.id_barang');
	}
	 public function scopeMaster($query, $req = []){

    $items = $query->join('ref_kategori', 'ref_kategori.id_kategori', '=', 'data_barang.id_kategori');
      if(!empty($req['kode']))
      $items->where('data_barang.kode', $req['kode']);
    if(!empty($req['barang']))
      $items->where('data_barang.nm_barang','LIKE', '%' . $req['barang'] . '%');
  	if(!empty($req['kategori']))
  		$items->where('data_barang.id_kategori', 'LIKE', '%' .$req['kategori']. '%');
    ...
    //Holil---\
   public function scopeMasterjasa($query, $req = []){
    $items = $query->join('ref_satuan', 'ref_satuan.id_satuan', '=', 'data_barang.id_satuan');
    if(!empty($req['kode']))
      $items->where('data_barang.kode', $req['kode']);
    if(!empty($req['barang']))
   ...
   public function scopeAxiz($query, $req = []){
	    $items = $query->join('ref_satuan', 'ref_satuan.id_satuan', '=', 'data_barang.id_satuan');
	    if(!empty($req['kode']))
	      $items->where('data_barang.kode', $req['kode']);
	    if(!empty($req['barang']))
	...
	public function batchs(){
		return $this->hasMany('App\Models\data_batch', 'id_barang');
	}
}
