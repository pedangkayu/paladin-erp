<?php
	...
	class Format{
		...
		///////////////////////////////// SELECT COA ///////////////////////////////
		public function select_coa($data, $parent = 0, $select = ""){
			static $i = 1;
	        $tab = str_repeat('&nbsp;&nbsp;&nbsp;', $i);
	        if(isset($data[$parent])){
	        	...
	        	foreach($data[$parent] as $item){
	        		$sel = $select == $item->id_coa ? 'selected="selected"' : '-';
	        		if($item->grup==1):

	        		$out .= '<option class="bolt" ' . $sel . ' value="'.$item->id_coa.'">';
	        		$out .= $tab . $item->kode . ' ' . $item->nm_coa;
	        		$out .= '</option>';
	        		else:


	        		$out .= '<option ' . $sel . '  value="'.$item->id_coa.'" label="' . $tab . $item->kode . ' ' . $item->nm_coa . '">';
	        		$out .= $item->kode . ' ' .$item->nm_coa;
	        		$out .= '</option>';

	        		endif;
	        		$anak = $this->select_coa($data, $item->id_coa, $select);
	        		if($anak){
						$out .= $anak;
						$i--;
	        		}
		...
		public function select_coa_ledger($data, $parent = 0, $select = ""){
			static $l = 1;
	        $tab = str_repeat('&nbsp;&nbsp;&nbsp;', $l);
	        if(isset($data[$parent])){
	        	$out = '';
	        	$l++;
	        	foreach($data[$parent] as $item){
	        		$sel = $select == $item->seri ? 'selected="selected"' : '';
	        		if($item->grup==1):

	        		$out .= '<option class="bolt" ' . $sel . ' value="'.$item->seri.'">';
	        		$out .= $tab . $item->kode . ' ' . $item->nm_coa;
	        		$out .= '</option>';
	        		else:


	        		$out .= '<option ' . $sel . '  value="'.$item->seri.'" label="' . $tab . $item->kode . ' ' . $item->nm_coa . '">';
	        		$out .= $item->kode . ' ' . $item->nm_coa;
	        		$out .= '</option>';

	        		endif;
	        		$anak = $this->select_coa_ledger($data, $item->id_coa, $select);
	        		if($anak){
						$out .= $anak;
						$l--;
	        		}
		...
		public function select_coa_faktur($data, $parent = 0, $select = ""){
			static $f = 1;
	        $tab = str_repeat('&nbsp;&nbsp;&nbsp;', $f);
	        if(isset($data[$parent])){
	        	$out = '';
	        	...
	        	foreach($data[$parent] as $item){
	        		...
	        		$out .= '<optgroup class="bolt" ' . $sel . ' value="'.$item->seri.'" label="' . $tab . $item->kode . ' ' . $item->nm_coa . '">';
	        		$out .= '</option>';
	        		else:
	        		...
	        		$anak = $this->select_coa_faktur($data, $item->id_coa, $select);
	        		if($anak){
						$out .= $anak;
						$f--;
	        		}
			...
			///////////select Coa group////////////////
		public function select_coa_group($data, $parent = 0, $select = 0){
			static $g = 1;
	        $tab = str_repeat('&nbsp;&nbsp;', $g);
	        if(isset($data[$parent])){
	        	$out = '';
	        	...
	        	foreach($data[$parent] as $item){
	        		if($item->grup==1):
	        			$sel = $select == $item->id_coa ? 'selected="selected"' : '';
	        		$out .= '<option ' . $sel . ' value="'.$item->id_coa.'">';
	        		$out .= $tab . $item->kode . ' ' . $item->nm_coa;
	        		$out .= '</option>';
	        		endif;
	        		$anak = $this->select_coa_group($data, $item->id_coa, 0);
	        		if($anak){
						$out .= $anak;
						$g--;
	        		}
