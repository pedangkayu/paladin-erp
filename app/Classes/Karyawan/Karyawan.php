<?php
	...
	class Karyawan {
		...
		//mohon jngan d hapus dulu karena untuk testing User roling
		public function subunit(){
			$unit = data_gudang_user::join('ref_gudang', 'ref_gudang.id_gudang', '=', 'data_gudang_user.id_gudang')
				->join('ref_unit','ref_unit.id_unit', '=', 'data_gudang_user.id_unit')
				->where('id_user', \Auth::user()->id_user)
				->select('data_gudang_user.id_gudang', 'ref_gudang.nm_gudang')
				->first();
			...
			if($unit == null){
				...
				$res['access']    = true;
				$res['id_gudang'] = $unit->id_gudang;
				$res['id_unit']   = $unit->id_unit;
				$res['nm_gudang'] = $unit->nm_gudang;
				$res['nm_unit']   = $unit->nm_unit;
		...
		public function dokter($id){
			if(empty($id)){
				...
				$dr = data_karyawan::find($id);
				if($dr == null)
					return "-";
				else
					return $dr->nm_depan . ' ' . $dr->nm_belakang;
			}
		}
	}