		...
		if($items->total() > 0){
			...
			foreach($items as $item){
					...
					<tr class="item_' .  $item->id . ' items">
						<td class="text-center"><img width="20" class="img-circle" src="'. asset('/img/avatars/xs/' . $item->avatar) .'"></td>
						<td>
						<a href="javascript:;" title="'. $item->name .'" data-toggle="tooltip" data-placement="bottom">'. $item->name .'</a>
							<div style="display:none;" class="tbl-opsi">
								<small>[
									<a href="'. url('/users/edit/' . $item->id_user) .'">Edit</a>
