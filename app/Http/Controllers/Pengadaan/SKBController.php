...
class SKBController extends Controller {
    ...
    public function getAllspb(Request $req){
        if($req->ajax()){
            ...
            foreach($items as $item){
                ...
                $btn = '';
                if($item->status == 2){
                     $btn = \Auth::user()->permission > 1 ? '<a href="javascript:;" onclick="ubahspb(\''. $item->id_spb .'\');" class="text-danger"><button class="btn btn-default btn-xs btn-mini">Selesai</button></a>' : '';
    ...
    public function postProcess(Request $req){
        ...
        if($skb['result'] == true){
            return redirect('/skb/batchview/' . $skb['id'])->withNotif([
                'label' => $skb['label'],
                'err' => $skb['err']
            ]);
        }else{
    ...
    public function getBatchview($id = 0){
        if(empty($id))
            return redirect('/skb');

         $skb = data_skb::byid($id);
        $items = data_skb_item::bybatch($id)->get();
        return view('Pengadaan.SKB.BatchView', [
            'skb' => $skb,
            'items' => $items
        ]);
    }
    ...
    public function getPrint($id){
        ...
        $find = data_spb::join('data_karyawan AS a', 'a.id_karyawan', '=', 'data_spb.id_pemohon')
            ...
            ->join('data_skb_item', 'data_skb_item.id_skb', '=', 'data_skb.id_skb')
            ->join('ref_gudang', 'ref_gudang.id_gudang', '=', 'data_skb_item.id_gudang')
     ...
     /*Mengubah status SPB*/
    public function postUbahspb(Request $req){
        if($req->ajax()){
     ...
     /* holil 21012016 */
     public function getCetakproseskb($id){
        $items = data_spb_item::cetakproses($id)->get();
        return view('Print.Pengadaan.Cetak', [
            'items' => $items,
            'satuan' => ref_satuan::all()
        ]);
	 }
