	...
	*/
	public function getDetail($id){
		...
		return view('Pengadaan.Detail',[
			'item' => $item,
			'details' => $details,
			'tipes' => [1=>'Obat',2=>'Barang',3=>'Inventaris']
		]);
	}
	/**
	...
	public function getGetusergudang(Request $req){
		if($req->ajax()){
			...
			foreach($users as $user){
				...
				$tipe3 =  $user->tipe == 3 ? 'checked="checked"' : '';
				...
				$out .= '
					<tr class="user-' . $user->id_akses_gudang . '">
						...
						<td>
							...
							<label for="inventaris' . $user->id_akses_gudang . '">
								<input type="radio" value="3" name="tipe' . $user->id_akses_gudang . '" onclick="edit(3, ' . $user->id_akses_gudang . ');" id="barang' . $user->id_akses_gudang . '" ' . $tipe3 . '>
