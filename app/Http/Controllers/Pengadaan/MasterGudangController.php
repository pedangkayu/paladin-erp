...
class MasterGudangController extends Controller {
		...
		if($req->ajax()){
			...
			if($items->total() > 0):
				...
				foreach($items as $item){
					$link = $permission > 2 ? '<small><a href="' . url('/gudang/edit/' . $item->id_gudang) . '">Edit</a> &middot; <a href="javascript:void(0);" onclick="del({{ $item->id_gudang }});" class="text-danger">Hapus</a></small>' : '';
					$out .= '
	...
	public function postIndex(Request $req){
		...
		return redirect()->back()->withNotif([
			'label' => 'success',
			'err' => 'Data Gudang berhasil dibuat dengan Kode. ' . $gd->kode_gudang
		]);
