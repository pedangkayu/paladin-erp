...
class SubGudangController extends Controller {
	...
	public function getGetusergudang(Request $req){
		if($req->ajax()){
			...
			foreach($users as $user){
				...
				$select = '<select style="width:100%;" onchange="edit(' . $user->id_gudang_user . ', this.value);">';
	    		...
	    		foreach($items as $item){
	    			$out .= '
						<tr class="item_'. $item->id_barang . '">
							<td width="20%">
								<a href="javascript:;" data-toggle="tooltip" data-placement="bottom" title="' . $item->id_barang . '">'. \Format::substr($item->nm_barang,25) .'</a>
							...
							<td width="10%" class="text-right">'. \number_format(($item->masuk - $item->keluar),0,',','.') .'  &nbsp;'. $item->nm_satuan .'</td>
							<td width="5%"><input type="number" min="0" name="qty[]" class="form-control text-right input-{{ $item->id_barang }}" ></td>
							<td class="satuan-item'. $item->id_barang .'">
									'.$item->nm_satuan.'
									<input type="hidden" value="'. $item->id_satuan .'" name="satuan[]">
							</td>
							<td width="20%">
    ...
    public function getLoaditem(Request $req){
        if($req->ajax()){
            $res = [];
            $out = '';
            $items = data_barang::axiz($req->all())->paginate(5);
            $total = $items->total();
            if($total > 0):
                foreach($items as $item){
                    $out .= '
                        <tr class="item-' . $item->id_barang . '">
                            ...
                            <td>' . $item->nm_barang . ' <small class="pull-right hide item-loading-loading-' . $item->id_barang . '">Memuat...</small></td>
    ...
    public function getCreateadj(){
        ...
        $items = data_item_gudang::join('data_barang', 'data_barang.id_barang', '=', 'data_item_gudang.id_barang')
            ...
            ->where('data_item_gudang.id_gudang', \Me::subgudang()->id_gudang)
    ...
    public function postCreateadj(Request $req){
        ...
        if(!empty($req['id_barang_t']) && count($req['id_barang_t']) > 0){
                    foreach ($req['id_barang_t'] as $a => $ab) {
                        $cekdata=data_item_gudang::where('data_item_gudang.id_barang',$ab)
                          ->where('data_item_gudang.id_gudang',$req['gudang'][$a])->first();
              if($cekdata == null){
                	foreach($req->jumlah_masuk as $jumlah_masuk){
        ...
        if(count($total) < 1)
            return redirect()->back()->withNotif([
                'label' => 'warning',
                'err' => 'New Qty tidak boleh semuanya kosong / Tidak boleh menambahkan Item yang sudah ada !'
            ]);
