...
class ReportLogistikController extends Controller {
   ...
   public function getItems(Request $req){
    ...
    public function getKs(Request $req){
        ...
        if($day > 91)
            return redirect()->back()->withNotif([
                'label' => 'danger',
                'err' => 'Pencarian tidak boleh lebih dari 90 hari!'
            ]);
    ...
    public function getLps(){
        $items = ref_gudang::listgudang()->get();
        return view('Laporan.Logistik.LPS', [
            'items' => $items
        ]);
    }

    public function getLpsajax(Request $req){
        if($req->ajax()):  /* Filter ajax */

            $res = [];
            $out = '';
            $gudang = $req->gudang == '' ? [] : $req->gudang;

            if(in_array('*', $gudang)){
                $field = '*';
            }else{
                $field = 'nm_barang AS `Nama Barang`, `harga_beli` AS `Harga Beli`, `jumlah`, `satuan`,`total`, `Gudang Besar`,';
                if($req->gudang != ''){
                    foreach ($req->gudang as $nama) {
                        $field .= '`' . $nama . '`,';
                    }
                }
            }

            $items = \DB::select('SELECT ' . rtrim($field,',') . ' FROM view_stok_akhir');
            $out .= '
                <table class="table table-striped">
            ';
                $out .= '<tr>';
                foreach ($items[0] as $field => $value) {
                    $out .= '<td>' . str_replace('nm_barang', 'Nama Barang', $field) . '</td>';
                }
                $out .= '</tr>';
                ...
                foreach ($items as $item) {
                     $out .= '<tr>';
                    foreach ($items[0] as $field => $value) {
                        $number = is_numeric($item->$field) ? number_format($item->$field,0,',','.') : $item->$field;
                        $out .= '<td>' . $number . '</td>';
                    }
                    $out .= '</tr>';
                }
        ...
        endif; /* End Filter ajax */
    }

    public function getLpgc(){
        $items = ref_gudang::listgudang()->get();
        return view('Laporan.Logistik.LaporanGudangKecil', [
            'items' => $items
        ]);
    }

    public function getLpgcajax(Request $req){
        if($req->ajax()):  /* Filter ajax */

            $res = [];
            $out = '';
            //$items = data_po_item::lpbdo($req->all())->paginate($req->limit);
            $items = data_log_barang::gudangkecil($req->all())->get();
                ...
                foreach ($items as $item) {
                    $out .= '
                        <tr>
                            ...
                            <td class="text-middle">' . \Format::indoDate2($item->created_at) . '</td>
                        </tr>
                    ';
                    $no++;
                }
    ...
    public function getLpgcprint(Request $req)
    {
         $items = data_log_barang::gudangkecil($req->all())->get();

         return view('Print.Laporan.Lpgcprint',[
            'items' => $items,
            'req' => $req
            ]);
    }
