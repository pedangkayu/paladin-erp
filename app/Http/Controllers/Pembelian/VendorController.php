...
class VendorController extends Controller {
	...
	public function getIndex(){
		...
		$data['items'] = data_vendor::listall()->paginate(10);
		
		return view('Pembelian.Vendor.DaftarVendors', $data);
	}
	...
	public function getFaktur($id = 0){
		...
		$items = data_faktur::forvendor($id, [])->paginate(10);
		...
		return view('Pembelian.Vendor.Faktur', [
			'vendor' => $vendor,
			'items' => $items,
			'status' => $status
		]);
	...
	public function getAllfaktur(Request $req){
		if($req->ajax()){
			...
			$items = data_faktur::forvendor($req->id, $req->all())->paginate($req->limit);
			if($items->total() > 0 ):
			$no = $items->currentPage() == 1 ? 1 : ($items->perPage() * $items->currentPage()) - $items->perPage() + 1;
			foreach($items as $item){
				$out .= '
