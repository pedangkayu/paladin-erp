...
class SPBMController extends Controller {
	...
	public function getOtheritems(Request $req){
		if($req->ajax()){
			$res = [];
			$out = '';
			...
			if($total > 0):
				...
				foreach ($items as $item) {
					$out .= '
						<tr class="other_tem_' . $item->id_barang . '"> 
							<td>' . $item->kode . '</td>
							<td>' . $item->nm_barang . '</td>
							<td>' . $tipe[$item->tipe] . '</td>
							<td>
								<button onclick="addotheritem(' . $item->id_barang . ')" class="btn btn-white btn-small"><i class="fa fa-plus"></i></button>
							</td>
						</tr>
					';
				}
	...
	public function getBonusother(Request $req){
		...
		$res['content'] = '
				<tr class="bonus-' . $req->id . ' item-bonus">
					<td title="' . $item->nm_barang . '">
						...
						<input type="hidden" name="id_satuan[]" value="' . $item->id_satuan . '">
						<input type="hidden" name="id_satuan_default[]" value="' . $item->id_satuan . '">
					</td>
					<td>
						<input type="text" data-exp="bonus" name="tgl_exp[]" class="form-control" value="' . date('Y-m-d', strtotime('+1 year')) . '" required="required" readonly="readonly" />
					</td>
