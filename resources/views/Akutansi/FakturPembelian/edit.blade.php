		...
		<div class="col-sm-12">
			...
			<div class="grid simple">
				<div class="grid-title no-border">
					<span class="label label-{{ $status[$faktur->status]['label'] }}"> {{ $status[$faktur->status]['err'] }} </span>
				</div>
							...
							<div class="row">
								<div class="col-sm-6">
									<div class="form-group">
										...
										<input type="hidden" name="total_ppn" value="{{ $faktur->total * $faktur->ppn / 100 }}">
						...
						<table class="table table-hover">
							...
							<tbody class="content-item">
								...
								<tr {{ $item->id_po > 0 ? 'data-po=true' : 'onclick="id_delete(\'items-' . $item->id_faktur_item . '\');"' }} class="item-barang" data-item="{{ $item->id_faktur_item }}">
									<td>
										{{ $item->kode }}
										<input type="hidden" value="{{ $item->id_faktur_item }}" name="id_faktur_item[]">
										<input type="hidden" value="{{ $item->deskripsi }}" name="nm_barang[]">
										<input type="hidden" value="{{ $item->id_item }}" name="id_barang[]">
										<input type="hidden" value="{{ $item->id_coa }}" name="id_coa[]">
