...
$(function(){
	$('#id_profesi').select2({
		placeholder: "Silahkan Pilih Profesi ..."
	});
...
<div class="col-md-12">
	<ul class="nav nav-tabs">
		<li class="active"><a href="{{ url('/karyawan/update/'.$karyawan->id_karyawan) }}">Biodata</a></li>
		<li><a href="{{ url('/karyawan/keluarga/'.$karyawan->id_karyawan) }}">Keluarga</a></li>
		<li><a href="{{ url('/karyawan/honor/'.$karyawan->id_karyawan) }}">Honor</a></li>
			...
			<form action="{{ url('karyawan\update') }}" method="post">
				...
				<div class="row column-seperation">
					<div class="col-md-6">
						...
						<div class="form-group">
							<div class="form-label">Profesi</div>
							<div class="control">
								<select class="form-control" id="id_profesi" name="id_profesi">
									<option value="">-Pilih-</option>
									@foreach($ref_profesi as $row)
									<option value="{{ $row->id_profesi }}"{{$row->id_profesi == $karyawan->id_profesi ? 'selected' :''}}> {{$row->nama_profesi }} </option>
									@endforeach
								</select>
							</div>
						</div>
